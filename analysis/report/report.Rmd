...
- **Aims.** Quantify bias, precision, coverage, and power of
...
- **Estimands.** The treatment effect on the slope,
  $\delta$, which under the generating model equals the
  mean difference in trajectory rate.
- **Methods.** SMA-slope, SMA-change, and MMRM (see Design
  specifications).
- **Performance measures.** Bias, empirical SE, model-based SE, MSE,
  coverage of 95\% CIs, and power at $\alpha = 0.05$, each reported
...
# n_reps is chosen via target MCSE. For coverage MCSE <= 1 pp at 0.95,
# need n_reps >= 475; for power MCSE <= 1 pp at p = 0.5, need n_reps
# >= 2500. Production runs use 1000; pilot runs can override via
# the N_REPS environment variable (e.g. N_REPS=100 for a fast
# integration smoke-test). The cache will rebuild on n_reps change.
...
For each of the 36 design conditions (3 arm sizes x 2 visit
counts x 3 missing-data rates x 2 missing-data mechanisms) and each
of 1,000 replications, we record the point estimate of the treatment
effect, its standard error, the p-value, and whether the 95\%
confidence interval covers the true value. Monte Carlo SEs are
...
hl_complete <- results |>
  dplyr::filter(miss_mech == "MCAR", miss_rate == 0)
...
Under complete data (MCAR, 0\% missing), all three
analytic procedures (SMA-slope, SMA-change, MMRM) recovered
the true treatment slope effect $\delta = 0.25$ with bias
below
...
Across the full factorial
(`r nrow(results)` scenario-method combinations), the
largest absolute bias observed was
...
missing,
`r hl_worst_bias$miss_mech` mechanism. Power ranged from
...
condition examined. Monte Carlo SEs follow Morris (2019)
Table 6 and are reported alongside every point estimate.
...
results |>
  ...
  knitr::kable(
    ...
    digits = 3,
    caption = paste(
      "Performance in the complete-data MCAR scenarios.",
      "Monte Carlo SEs from Morris et al. (2019) Table 6."
    )
  ) |>
...
structure; see `analysis/data/derived_data/` for the full
results data frame.

## Figures

```{r power-plot, eval=TRUE, fig.cap="Power comparison across methods and design conditions.", fig.width=6, fig.height=4}
results |>
...
```{r coverage-plot, eval=TRUE, fig.cap="95\\% CI coverage across methods and design conditions.", fig.width=6, fig.height=4}
results |>
  filter(miss_mech == "MCAR") |>
  ggplot(aes(
    x = factor(n_per_arm),
...
**Verdict at submission:** Compliant. The audit predates the
...
- *No Monte Carlo SE machinery* — Resolved. The
  `summarize_results()` function in `sim_study.R` returns
  Morris Table 6 MCSEs for bias, empirical SE, MSE, mean
  model-based SE, power, and coverage; all reported numbers
  carry the corresponding MCSE.

Pilot runs use `N_REPS=100` for fast smoke-test rendering;
production submission uses the default `n_reps = 1000`.
