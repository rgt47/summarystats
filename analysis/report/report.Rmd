...
#
# RESOLVED (remediation 2026-08-20, second pass): this chunk was
# re-rendered at the production n_reps = 1000 over the full
# corrected design grid (30 non-null-missingness conditions x 2
# delta values). That run also exposed and fixed a second bug in
# `fit_mmrm()` (see `sim_study.R`): the p-value was read from
# column 4 of `summary(mmrm_fit)$coefficients`, which is the t
# value, not column 5 (`Pr(>|t|)`); this produced an MMRM Type I
# error near 0.50 at every design condition in the first
# production-scale attempt, undetected in the earlier n_reps = 15
# pilot because the pilot's headline table only reports
# delta = 0.25 cells and its wide Monte Carlo SE masked the
# problem in the delta = 0 cells that were computed. The fix
# (`pval <- s[idx, 5]`) is regression-tested in
# `inst/tinytest/test_basic.R`. `sim_09.rds` below now holds the
# real production-scale output. To regenerate it, run:
...
# with `N_REPS=1000` set in the environment; expect roughly
# 1.5-2 hours on a single host core.
...
# NOTE (remediation 2026-08-20, second pass): `results` in the
# currently committed `sim_09.rds` is the production-scale run
# (n_reps = 1000 over the full design grid). The numbers below are
# final submission-quality estimates rather than pilot illustrations.
hl_complete <- results |>
...
ranges are computed from the production-scale run (`n_reps = `r
n_reps``) and carry the Monte Carlo SEs reported alongside every
point estimate in the derived-data object. The SMA methods are
slightly conservative relative to nominal 0.05 throughout; MMRM
is close to nominal in most conditions but modestly inflated
(up to about 0.10) in the smallest-arm-size, most-visits
condition ($n_{\mathrm{arm}} = 30$, $p = 8$), consistent with
known small-sample behavior of Satterthwaite degrees of freedom
under an unstructured covariance with many parameters relative
...
This document was rendered with `n_reps = `r n_reps``
replications per design condition, seed `20260310`
(`RNGkind("L'Ecuyer-CMRG")`), writing
`analysis/data/derived_data/sim_09.rds` and per-condition RNG
state sidecars under `analysis/data/derived_data/rng_states/`.
To reproduce this run, set `N_REPS=1000` in the environment and
render via `bash tools/render.sh analysis/report/report.Rmd` (or
`N_REPS=1000 Rscript -e "rmarkdown::render('analysis/report/report.Rmd')"`
if the render wrapper is unavailable); a full run over the design
grid takes on the order of an hour or more.
...
**Verdict as of this remediation pass (2026-08-20, second pass):
...
Table 6 Monte Carlo SE. The currently committed `sim_09.rds` (and
therefore every inline number, table, and figure in this
document) is the production-scale run, `n_reps = 1000` across the
full 60-condition design grid, so the reported Monte Carlo SEs
(about 0.7 percentage points for coverage, about 1.4-1.6
percentage points for power in the least favorable cells) are
informative at the precision Morris et al.'s reporting standard
presumes. This production run also surfaced a second coding
defect not visible in the pilot: `fit_mmrm()` read the t
statistic rather than the p-value from `mmrm`'s coefficient
table (column 4 instead of column 5), which produced an MMRM
Type I error near 0.50 in an initial production-scale attempt.
The fix is regression-tested (`inst/tinytest/test_basic.R`) and
the run redone; see the `run-sim` chunk in the Methods for
detail.
...
- *MMRM total fit failure (M1)* — Resolved: the covariance term
  ...
  verified nonzero (at least 0.998 in every reported cell) in the
  production-scale run (see the tinytest suite in
  `inst/tinytest/test_basic.R` for a regression check).
...
- *No Type I error assessment (M4)* — Resolved: `delta = 0` is
  ...
  subsection.
- *Production-scale run not yet executed* — Resolved: the
  committed `sim_09.rds` is the `n_reps = 1000` production run
  over the full design grid.
- *MMRM p-value read from the wrong `mmrm::summary()` coefficient
  column (found during the production-scale run)* — Resolved:
  `fit_mmrm()` now reads `Pr(>|t|)` (column 5) rather than
  `t value` (column 4); regression-tested in
...
the full design grid, as used to generate the results reported
in this document.
