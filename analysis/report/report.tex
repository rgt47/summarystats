...
\renewcommand{\stamptime}{2026-08-20 22:28 PDT}
\renewcommand{\stampversion}{\detokenize{7fca2d3}}
\usepackage{amsmath}
\usepackage{amssymb}
\usepackage{booktabs}
...
0.047 to
0.099. These
ranges are computed from the production-scale run (\texttt{n\_reps\ =}r
n\_reps``) and carry the Monte Carlo SEs reported alongside every
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={\label{fig:power-plot}Power comparison across methods and design conditions at \textbackslash delta = 0.25.}]{report_files/report_files/figure-latex/power-plot-1.pdf}}
\caption{\label{fig:power-plot}Power comparison across methods and design conditions at \(\delta = 0.25\).}
\end{figure}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={\label{fig:coverage-plot}95\% CI coverage across methods and design conditions at \textbackslash delta = 0.25, each method scored against its own estimand.}]{report_files/report_files/figure-latex/coverage-plot-1.pdf}}
...
This document was rendered with \texttt{n\_reps\ =\ 1000}
replications per design condition, seed \texttt{20260310}
(\texttt{RNGkind("L\textquotesingle{}Ecuyer-CMRG")}), writing
\texttt{analysis/data/derived\_data/sim\_09.rds} and per-condition RNG
state sidecars under \texttt{analysis/data/derived\_data/rng\_states/}.
To reproduce this run, set \texttt{N\_REPS=1000} in the environment and
render via \texttt{bash\ tools/render.sh\ analysis/report/report.Rmd} (or
\texttt{N\_REPS=1000\ Rscript\ -e\ "rmarkdown::render(\textquotesingle{}analysis/report/report.Rmd\textquotesingle{})"}
...
\textbf{Verdict as of this remediation pass (2026-08-20, second pass):
...
Table 6 Monte Carlo SE. The currently committed \texttt{sim\_09.rds} (and
therefore every inline number, table, and figure in this
document) is the production-scale run, \texttt{n\_reps\ =\ 1000} across the
...
defect not visible in the pilot: \texttt{fit\_mmrm()} read the t
statistic rather than the p-value from \texttt{mmrm}'s coefficient
table (column 4 instead of column 5), which produced an MMRM
Type I error near 0.50 in an initial production-scale attempt.
The fix is regression-tested (\texttt{inst/tinytest/test\_basic.R}) and
the run redone; see the \texttt{run-sim} chunk in the Methods for
detail.
...
\item
  ...
  \texttt{inst/tinytest/test\_basic.R} for a regression check).
\item
...
\item
  ...
  subsection.
\item
  \emph{Production-scale run not yet executed} --- Resolved: the
  committed \texttt{sim\_09.rds} is the \texttt{n\_reps\ =\ 1000} production run
  over the full design grid.
\item
  \emph{MMRM p-value read from the wrong \texttt{mmrm::summary()} coefficient
  column (found during the production-scale run)} --- Resolved:
  \texttt{fit\_mmrm()} now reads \texttt{Pr(\textgreater{}\textbar{}t\textbar{})} (column 5) rather than
  \texttt{t\ value} (column 4); regression-tested in
  \texttt{inst/tinytest/test\_basic.R}.
\end{itemize}
